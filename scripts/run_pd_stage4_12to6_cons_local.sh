#!/usr/bin/env bash
# Stage-4 PD cons: 12→6 constraint-first (same hypers as cons/step_4000 peak).
# Default OUT=stage_12to6_cons_rerun so prior stage_12to6_cons/ is preserved.
set -euo pipefail
...
OUT="${OUT:-outputs/pd_g1_rp_teacher_seed/stage_12to6_cons_rerun}"
